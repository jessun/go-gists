...
import (
	...
	commonUser "actiontech.cloud/universe/ucommon/v4/user"
)
...
var rmExp = "^\\d{4}_(\\d{2}_){5}.*\\.log.tar.gz$"
...
func NewFileLogger(logDir, configPath string, fileLimit, totalLimit int, currentUser string, enableDetail bool, enableHouseKeep bool) *fileLogger {
	ret := fileLogger{}
	ret.dir = logDir
	ret.fileLimit = fileLimit
	ret.totalLimit = totalLimit
	ret.traceFilter = ""
	ret.fileOwner = currentUser
	...
	ret.setLevel(detail, 10, enableDetail, "/detail.log")
	ret.loadDynamicPropertiesLoop()
	if enableHouseKeep {
		ret.zipLoop()
		ret.removeLoop()
	}
...
func newLogger(dir, configPath string) *fileLogger {
	ret := fileLogger{}
	ret.dir = dir
	ret.fileLimit = 100
	ret.totalLimit = 1000
...
func (logger *fileLogger) newLogFile(level int) (*os.File, error) {
	...
	if "root" != logger.fileOwner {
		ownerId := fi.Sys().(*builtin_syscall.Stat_t).Uid
...
func (logger *fileLogger) printLog(level int, line string) {
	...
	var writers = []io.Writer{file}
	// Check whether it is within the k8s cluster
	host, port := os.Getenv("KUBERNETES_SERVICE_HOST"), os.Getenv("KUBERNETES_SERVICE_PORT")
	if level == key && len(host) > 0 && len(port) > 0{
		writers = append(writers, os.Stdout)
	}
	fileAndStdoutWriter := io.MultiWriter(writers...)
	if _, err := fileAndStdoutWriter.Write([]byte(strings.Join([]string{line, "\n"},""))); nil != err {
		file.Close()
		logger.levels[level].fileFd = nil
		return
	}
...
func (logger *fileLogger) loadDynamicProperties() {
	...
	if nil != err {
		if !strings.HasPrefix(err.Error(), "option 'enable_detail' not found in section 'logger'") { //NO_DUPL_CHECK
