...
var hideFiles = map[string] /*fileName*/ struct{}{
	"asm_amd64.s": {},
}

func isNeedHide(fileName string) (shouldHide bool) {
	if _, ok := hideFiles[fileName]; ok {
		return true
	}
...
var sample = []string{ // NO_DUPL_CHECK
	...
	for _, key := range sample {
		if idx := strings.Index(line, key); -1 != idx {
			...
			if line[s-1] == '>' {
				...
				for i < len(line) && line[i] != '<' { //NO_DUPL_CHECK
...
func (b *Builder) Debug(msg string, args ...interface{}) *Builder { //NO_DUPL_CHECK
	if shouldPrintLog(user) {
		return b.buildLine(user, "[DEBUG] "+msg, args...)
	}
	return b
}

func (b *Builder) Errorf(msg string, args ...interface{}) *Builder {
	if shouldPrintLog(user) {
...
func (b *Builder) UserInfo(msg string, args ...interface{}) *Builder { //NO_DUPL_CHECK
	if shouldPrintLog(user) {
...
func (b *Builder) Brief(msg string, args ...interface{}) *Builder { //NO_DUPL_CHECK
...
func (b *Builder) DetailSystem(msg string, args ...interface{}) *Builder {
	if shouldPrintLog(detail) {
		return b.buildLine(detail, "[SYSTEM] "+msg, args...)
	}
	return b
}
