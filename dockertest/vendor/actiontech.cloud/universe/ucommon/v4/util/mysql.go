...
func (s *SQLGen) AddFuzzyOrCondition(key, value string, loc fuzzyLoc) *SQLGen { // NO_DUPL_CHECK
...
func (s *SQLGen) AddFuzzyAndCondition(key, value string, loc fuzzyLoc) *SQLGen { // NO_DUPL_CHECK
	fuzzyValue := s.getFuzzyValue(value, loc)
...
func (s *SQLGen) OffSet(offset int) *SQLGen { // NO_DUPL_CHECK
	s.offset = uint32(offset)
	return s
}

func (s *SQLGen) Limit(limit int) *SQLGen { // NO_DUPL_CHECK
...
func (s *SQLGen) GenerateSQL() (countSql string /*countSql*/, querySql string /*querySql*/) {
	...
	if len(s.fuzzyOrKeys) != 0 { // NO_DUPL_CHECK
		s.query.WriteString(fuzzyQueryConcat(s.fuzzyOrKeys, s.fuzzyOrValues, " OR "))
	}

	if len(s.fuzzyAndKeys) != 0 { // NO_DUPL_CHECK
