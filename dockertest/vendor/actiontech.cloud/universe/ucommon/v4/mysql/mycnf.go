...
func (t *Mycnf) GetKeyInMycnf(option string) string {
	...
	for _, key := range keys {
		keyAfterReplace := strings.Replace(key, "-", "_", -1)
		if keyAfterReplace == option || keyAfterReplace == "loose_"+option {
			return key
		}
	}
	return ""
...
func (t *Mycnf) SetUniverseOption(key, newVal string) {
	t.SetOption("universe", key, newVal)
}

// BulkSetUniverseOption bulk saves options to [Universe] section of a Mycnf
func (t *Mycnf) BulkSetUniverseOption(options map[string]interface{}) {
	for k, v := range options {
		t.SetUniverseOption(k, fmt.Sprintf("%v", v))
	}
}
...
func (t *Mycnf) RemoveUniverse() {
	r, _ := t.reader()
	if !r.HasSection("universe") {
		return
	}
	reg := `(?s)(\[universe\].*\n)\[\S+\]`
	subStrs := regexp.MustCompile(reg).FindStringSubmatch(t.content)
	t.content = strings.Replace(t.content, subStrs[1], "", -1)
}
...
func (t *Mycnf) DelSection(section string) {
	r, _ := t.reader()
	if r.HasSection(section) {
		r.RemoveSection(section)
		t.content = string(r.WriteConfigBytes(""))
	}
}

func (t *Mycnf) DelUniverseSection() {
	t.DelSection("universe")
}

// RemoveNoAutoRehashEmptyValue
// @title       移除no-auto-rehash空默认值
// @description 如果后续有需要移除其他空默认值的话可以调整该方法，将需要移除的key通过入参传入
// @update_time 2021-09-14 15:00:43
// @receiver    t     *Mycnf
// @return      mycnf *Mycnf 配置信息
func (t *Mycnf) RemoveNoAutoRehashEmptyValue() *Mycnf {
	if strings.Contains(t.String(), "no-auto-rehash = \n") {
		t.content = strings.Replace(t.String(), "no-auto-rehash = \n", "no-auto-rehash\n", -1)
	}
	return t
}
