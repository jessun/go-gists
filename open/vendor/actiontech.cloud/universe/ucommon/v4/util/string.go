...
func StringNvl(strs ...string) string { //NO_DUPL_CHECK
...
func StringSliceEqual(a, b []string) bool { //NO_DUPL_CHECK
	if len(a) != len(b) {
		return false
	}
	for idx, _ := range a {
...
// Generate password that can be passed the mysql validate plugin
func GeneratePassword() string {
	password := RandStr("", "abcdefghijklmnopqrstuvwxyz", 3)
	password += RandStr("", "ABCDEFGHIJKLMNOPQRSTUVWXYZ", 3)
	password += RandStr("", "123456789", 1)
	password += RandStr("", "@#$%^*+-=_", 1)
	return password
}

// StringsSubtract remove any string that A,B both contain from A and returns the remainder of A
func StringsSubtract(A, B []string) []string {
...
// StringsRemoveEmpty removes all empty string from a slice.
