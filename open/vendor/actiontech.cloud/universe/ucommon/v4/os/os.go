...
// IsBindSip checks a CIDR's binding status when the mask argument is not empty,
// otherwise it checks only the sip
func IsBindSip(stage *log.Stage, ip, ipMaskSize string) bool {
	if ipMaskSize == "" {
		ipMaskSize = DefaultIPMaskSize
	}
	return HasLocalIpWithMaskSize(stage, ip, ipMaskSize)
}

// HasLocalIpWithMaskSize can tell if a CIDR binds to the local machine
func HasLocalIpWithMaskSize(stage *log.Stage, ip, ipMaskSize string) bool {
	interfaces, err := net.Interfaces()
	if err != nil {
		log.Detail(stage, "failed to get net interfaces: %v", err)
		return false
	}
	for _, i := range interfaces {
		addrs, err := i.Addrs()
		if err != nil {
			log.Detail(stage, err.Error())
		}
		if addrInSlice(ip, ipMaskSize, addrs) {
			return true
		}
	}
	return false
...
// addrInSlice can tell if a network end point address resides in a slice of net.Addr
func addrInSlice(ip, ipMaskSize string, addrs []net.Addr) bool {
	ipWithMask := fmt.Sprintf("%v/%v", ip, ipMaskSize)
	for _, addr := range addrs {
		if addr.String() == ipWithMask {
			return true
		}
	}
	return false
...
func HaltIfShutdown(stage *log.Stage,dir string) {
	rebootPath := filepath.Join(dir, ".universe_is_reboot")
	if IsFileExist(rebootPath) {
