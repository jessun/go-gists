...
func NewHaCmd(stage *log.Stage, str string) (a *HaCmd, err error) {
	...
	// ref: syscall_test.mustSupportAmbientCaps
	var uname builtin_syscall.Utsname
	if err := builtin_syscall.Uname(&uname); err != nil {
		return nil, err
	}
	var buf [65]byte
	...
	rel := strings.TrimSpace(string(buf[:]))

	if rel != "" &&
		!(strings.HasPrefix(rel, "2.") ||
			strings.HasPrefix(rel, "3.") ||
			strings.HasPrefix(rel, "4.1.") ||
			strings.HasPrefix(rel, "4.2.")) { //ambient capabilities is introduced after Kernel 4.3
		...
		log.KeyDilute2(stage, str, "exec [%v] with AmbientCaps[%+v]", str, uintCaps)
	} else {
		log.KeyDilute2(stage, str, "exec [%v] without AmbientCaps", str)
	}
	return a, nil
}
